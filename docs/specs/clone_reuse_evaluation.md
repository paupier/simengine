...
> **Historical planning document.** The carry-over analysis for `recipe_runner.py` and other recipe machinery below no longer applies — recipes were removed after this was written and are not part of the current engine. See `CLAUDE.md` and `README.md` for current state.
